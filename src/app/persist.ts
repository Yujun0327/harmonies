...
export function playerKey(): string {
  const KEY = 'harmonies:player-key'
...
export function loadPlayerName(): string {
  return localStorage.getItem('harmonies:player-name') ?? ''
}

export function savePlayerName(name: string): void {
  localStorage.setItem('harmonies:player-name', name)
}
...
const roomKey = (room: string, key: string) => `harmonies:room:${room.toUpperCase()}:${key}`
