...
import type { SfxEvent } from '../ui/audio'
...
export abstract class BaseSession {
  ...
  /** Harmonies is fully open information — redact is the identity. */
  get visibleState(): GameState {
    return redact(this.state, this.mySeat)
  }

  /** Harmonies has no mid-turn interrupts: the actor is always the turn holder. */
  get actor(): Seat {
  ...
  protected applyLocal(actor: Seat, move: Move): void {
    ...
    if (move.type === 'takeSlot') this.emit('take')
    if (move.type === 'placeToken' && move.hex !== null) this.emit('place')
    if (move.type === 'takeCard') this.emit('card')
    if (move.type === 'placeCube') {
      const completedCount = (s: GameState) =>
        s.players.reduce((n, p) => n + p.completed.length, 0)
      this.emit(completedCount(after) > completedCount(before) ? 'complete' : 'cube')
    }
...
/** Current totals for every seat, for scoreboards. */
export function scores(state: GameState): number[] {
  return state.players.map((p) => scoreBreakdown(p).total)
}
...
/**
 * N-player sync via turn-holder sequencing: Harmonies never has concurrent
...
export class OnlineSession extends BaseSession {
  ...
  private onHello(msg: NetMsg & { t: 'hello' }, from: string): void {
    ...
    if (msg.protocol !== PROTOCOL_VERSION) {
      console.warn('[harmonies] protocol mismatch', msg.protocol)
      this.status = 'desync'
      return
    }
