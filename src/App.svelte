<script lang="ts">
  ...
  function syncFromHash() {
    ...
    if (room && online?.room !== room) {
      online?.destroy()
      const creator = sessionStorage.getItem(`harmonies:creator:${room}`) !== null
  ...
  $effect(() => {
    ;(window as unknown as Record<string, unknown>).__harmonies = online ?? hotseat
  })
  ...
  function createRoom() {
    ...
    sessionStorage.setItem(`harmonies:creator:${code}`, '1')
